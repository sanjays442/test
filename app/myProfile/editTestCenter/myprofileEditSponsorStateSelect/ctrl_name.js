...
function ctrl($document, $log, $rootScope, $injector, $state, UIState, service, centerService, localStorageService, Status, $timeout, mapService) {
  ...
  // preselected items
  $rootScope.preselectedItems = [];
  $rootScope.newItemsPaymentTotal = 0;

  // pre load from cart item
  vm.setCategoryCartData = function (cartMode) {
    var actCen = cartMode.data.id;
    var catgs = [];
    for (var key in cartMode.data.sponsored_listing_layouts.categories) {
      //  $rootScope.treatmentApproachModel[actCen].push();
      catgs.push(cartMode.data.sponsored_listing_layouts.categories[key].name);
    }

    var demographic = $rootScope.otherIds.Demographic;
    // check if not coming from do payment back button
    if (angular.isUndefined($rootScope.previousStep) || $rootScope.previousStep !== 'doPayment') {
    ...
    for (var key in demographic) {
      if (catgs.indexOf(demographic[key].name) >= 0) {
        ...
        $rootScope.preselectedItems.push(demographic[key].id);
      }
    }
    ...
    for (key in treatmentApproach) {
      if (catgs.indexOf(treatmentApproach[key].name) >= 0) {
        ...
        $rootScope.preselectedItems.push(treatmentApproach[key].id);
      }
    }
    ...
    for (key in setting) {
      if (catgs.indexOf(setting[key].name) >= 0) {
        ...
        $rootScope.preselectedItems.push(setting[key].id);
      }
    }
    ...
    for (key in additionalServices) {
      if (catgs.indexOf(additionalServices[key].name) >= 0) {
        ...
        $rootScope.preselectedItems.push(additionalServices[key].id);
      }
    }
    ...
    for (key in payment) {
      if (catgs.indexOf(payment[key].name) >= 0) {
        ...
        $rootScope.preselectedItems.push(payment[key].id);
      }
    }
    ...
    for (key in byDrug) {
      if (catgs.indexOf(byDrug[key].name) >= 0) {
        ...
        $rootScope.preselectedItems.push(byDrug[key].id);
      }
    }
  };
  ...
  function setOtherDemography() {
    ...
    vm.cartMode = localStorageService.get('cartModeEdit');
  ...
  // removing localstorage data of sponsoredpage to prevent data to be picked from localstorage data
  ...
  vm.updateCategoriesModel = function (value, catgType) {
    ...
    if (catgType === 'demographic') {
      ...
      if (matched === 1) {
        ...
        $rootScope.newItemsPaymentTotal -= $rootScope.sponsorPricingInfo.price_sponsored;
      } else {
  ...
  vm.citySelectFun = function (item) {
    ...
    saveToLocalStorage($rootScope, localStorageService);
    $rootScope.onInit();
    $rootScope.newItemsPaymentTotal += $rootScope.sponsorPricingInfo.price_city;
  };
  ...
  vm.countySelectFun = function () {
    ...
    saveToLocalStorage($rootScope, localStorageService);
    $rootScope.onInit();
    $rootScope.newItemsPaymentTotal += $rootScope.sponsorPricingInfo.price_county;
  };
  ...
  vm.updateStateSelect = function (state, stateCheck) {
    ...
    var stateShortName = state.name ? state.name : state.shortname;
  ...
  vm.stateCheckboxClick = function (state, selectOnly, mapClicked, deselectAll) {
    ...
    var shortname = state.shortname ? state.shortname : state.name;
    ...
    if (selectOnly === true) {
      ...
      if (angular.isDefined(vm.stateSelectCheck[shortname]) && vm.stateSelectCheck[shortname] === false) {
        ...
        saveToLocalStorage($rootScope, localStorageService);
        $rootScope.newItemsPaymentTotal += $rootScope.sponsorPricingInfo.price_state;
        //  $rootScope.onInit();
  ...
  vm.loadCityCounty = function (state, mapOnly) {
    ...
    state.shortname = state.name ? state.name : state.shortname;
    state.name = state.name ? state.name : state.shortname;
  ...
  vm.cartEditMode = function () {
    ...
    if (angular.isDefined(cartMode) && cartMode !== null) {
      ...
      if (cartMode.mode === 'edit' && cartMode.item === 'sponsored_layouts') {
        ...
        var counts = [];
        var stats = [];
        for (var key in cartMode.data.sponsored_listing_layouts.city) {
          //  cits.push(cartMode.data.sponsored_listing_layouts.city[key].name);
          $rootScope.preselectedItems.push(cartMode.data.sponsored_listing_layouts.city[key].id);
        }
        for (var key in cartMode.data.sponsored_listing_layouts.county) {
          //  counts.push(cartMode.data.sponsored_listing_layouts.county[key].name);
          $rootScope.preselectedItems.push(cartMode.data.sponsored_listing_layouts.county[key].id);
        }
        // check if not coming from do payment back button
        if (angular.isUndefined($rootScope.previousStep) || $rootScope.previousStep !== 'doPayment') {
        ...
        for (var key in cartMode.data.sponsored_listing_layouts.state) {
          //  $rootScope.treatmentApproachModel[actCen].push();
          stats.push(cartMode.data.sponsored_listing_layouts.state[key].name);
        }
        ...
        for (key in $rootScope.stateIds) {
          if (stats.indexOf($rootScope.stateIds[key].name) >= 0) {
            ...
            $rootScope.preselectedItems.push($rootScope.stateIds[key].id);
          }
        }
        saveToLocalStorage($rootScope, localStorageService);
        $rootScope.onInit();
  ...
  vm.selectAllState = function (selectall, key) {
    ...
    if (selectall) {
      ...
      if (angular.isDefined(vm.stateSelectCheck[shortname]) && vm.stateSelectCheck[shortname] === false) {
        vm.stateCheckboxClick(stateSelectedData, true, false);
        $timeout(function () {
          vm.selectAllState(selectall, (key + 1));
        }, 80);
        // vm.stateSelectCheck[shortname] = true;
...
function getCountyCity(vm, state, stateMap, service, $injector, $rootScope, localStorageService, Status) {
  ...
  console.log($rootScope.preselectedItems);
  service.getCityCountyByStateV2(state.shortname).then(function (response) {
    ...
    for (var key in response.city) {
      if ($rootScope.preselectedItems.indexOf(response.city[key].id) >= 0) {
        disabledVal = true;
      } else {
        disabledVal = false;
      }
    ...
    for (key in response.county) {
      if ($rootScope.preselectedItems.indexOf(response.county[key].id) >= 0) {
        disabledVal = true;
      } else {
        disabledVal = false;
      }
        ...
        for (key in modifiedCitySelect) {
          if (cits.indexOf(modifiedCitySelect[key].label) >= 0) {
            ...
            if (matched === 0) {
              ...
              //  $rootScope.preselectedItems.push(modifiedCitySelect[key].id);
            }
          }
        }
        //  $rootScope.countyModel = {};
        ...
        for (key in modifiedCountySelect) {
          if (counts.indexOf(modifiedCountySelect[key].label) >= 0) {
            ...
            if (matched === 0) {
              ...
              //  $rootScope.preselectedItems.push(modifiedCountySelect[key].id);
            }
          }
        }
        // saving
...
function saveToLocalStorage($rootScope, localStorageService) {
  ...
  if (localStorageService.isSupported) {
    console.log('saving to localstorage temporary stopped here');
