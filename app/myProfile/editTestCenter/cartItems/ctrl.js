...
function ctrl($injector, $timeout, $scope, $log, $rootScope, $state, Status, UIState, service, localStorageService) {
  ...
  $rootScope.unpaidItemsCount = 0; // use in header cart icon
  // ------------------------ cart related code ----------------------------------- //
  vm.paymentInfo = localStorageService.get('paymentActionInfo');

  vm.goBack = function () {
    if (angular.isDefined(vm.paymentInfo) && vm.paymentInfo !== null) {
  ...
  vm.loadCart = function (cenId, toggle) {
    ...
    service.getCartDetails().then(function (result) {
      ...
      if (angular.isDefined(vm.cartDetails.items) && vm.cartDetails.items.length === 0) {
        vm.cartError = 1;
      }
