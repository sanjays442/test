...
function ctrl($location, $anchorScroll, $injector, $document, $scope, $log, $rootScope, $state, UIState, service, localStorageService, Status) {
  ...
  if ((bannerAdInfo === null || bannerAdInfo === '') && (angular.isUndefined($rootScope.previousStep) || $rootScope.previousStep !== 'doPayment')) {
  ...
  $scope.uploadChange = function (element, position) {
    ...
    vm.editContentChange.push(position); // helps to know which ads have new image uploaded
  };
  ...
  vm.submitAds = function (content, position, weblink, id) {
    ...
    if (angular.isUndefined(id)) {
      ...
      if (vm.count >= vm.validAds.length) {
        ...
        //    $state.go(UIState.MY_PROFILE.DO_PAYMENT); //enable these lines if want to execute add api after payemnt in payment controller
      }
    ...
    if (vm.editContentChange.indexOf(position) >= 0) {
      publishAds = {
        'content': content,
        // 'position': position,
        'center_web_link': weblink
    ...
    service.bannerAdsEdit(formData, centerId, id).then(function (result) {
      ...
      if (vm.count >= vm.validAds.length) {
        ...
        // vm.checkSponsorGo();
        if (addModeExist === 0 && unpaidAdsExist === 0) {
          localStorageService.remove('editBannerAdsInfo', 'sessionStorage');
        }
        lm.$emit(Status.HIDE_PROCESSING, '');

        if ((angular.isDefined(vm.newAdsPending) && vm.newAdsPending.data.length > 0) || unpaidAdsExist === 1) {
          $state.go(UIState.MY_PROFILE.DO_PAYMENT);
        } else {
          lm.$emit(Status.SUCCEEDED, 'Updated');
          vm.goHome();
        }
      }
      vm.count++;
  ...
  vm.populateValues = function () {
    if (bannerAdInfo.mode !== 'edit') {
      vm.hideAdsHtml.header = 1;
      vm.hideAdsHtml.sidebar = 1;
      vm.hideAdsHtml.footer = 1;
    }
    ...
    if (angular.isDefined(vm.adsInfo)) {
      ...
      for (var key in publishAds) {
        ...
        if (bannerAdInfo.mode === 'edit' && bannerAdInfo.ads.id != publishAds[key].id) {
          continue;
        }

        var adPos = publishAds[key].position;
        //check if any unpaid ads exist
        if (publishAds[key].active === false) {
          vm.unpaidAds[adPos] = 1;
        } else {
          vm.unpaidAds[adPos] = 0;
        }
        if (adPos === 'header') {
