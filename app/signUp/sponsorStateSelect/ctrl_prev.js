...
function ctrl($log, $document, $rootScope, $injector, $state, UIState, service, signupService, localStorageService, Status, $timeout, mapService) {
  ...
  $rootScope.loadModelsCenterwise = function () {
    ...
    if (angular.isUndefined(vm.checkAllText) || vm.checkAllText === null) {
      ...
      for (cen in $rootScope.treatmentCentersValue) {
        ...
        console.log('test' + vm.checkAllText[centerId]);
      }
    }
  ...
  vm.open = function (state, checkboxClick) {
    ...
    vm.stateCheckboxClick(state, true);
  ...
  vm.stateCheckboxClick = function (state, selectOnly) {
    ...
    if (selectOnly === true) {
      // vm.prevSelStyle = {};
      vm.curSelStyle = {};
      vm.curSelStyle[shortname] = 'color:#428bca;text-decoration:underline;';
      ...
      if (angular.isDefined(vm.stateSelectCheck[shortname]) && vm.stateSelectCheck[shortname] === false) {
        ...
        vm.prevSelStyle[shortname]  = 'background-color:#c3e09f;';
  ...
  vm.selectAllState = function (selectall) {
    if (selectall) {
      for (key in $rootScope.stateIds) {
        $log.info($rootScope.stateIds[key].name);
        stateSelectedData = {
