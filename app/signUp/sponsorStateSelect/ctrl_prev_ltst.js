...
function ctrl($log, $document, $rootScope, $injector, $state, UIState, service, signupService, localStorageService, Status, $timeout, mapService) {
  ...
  vm.open = function (state, checkboxClick) {
    ...
    // $log.info('checkboxclieck: ' + checkboxClick);
    // if (checkboxClick === false) {
    // // if state already selected else make state selected
    //   if (angular.isDefined(vm.stateSelectCheck) && angular.isDefined(vm.stateSelectCheck[state.shortname]) && vm.stateSelectCheck[state.shortname] === true) {
    //     vm.stateSelectCheck[state.shortname] = false;
    //     vm.updateStateSelect(state, false);
    //   // save to localStorageService
    //     saveToLocalStorage($rootScope, localStorageService);
    //     $rootScope.onInit();
    //   } else if (angular.isDefined(vm.stateSelectCheck) && angular.isDefined(vm.stateSelectCheck[state.shortname]) && vm.stateSelectCheck[state.shortname] === false) {
    //     vm.stateSelectCheck[state.shortname] = true;
    //     vm.updateStateSelect(state, true);
    //   // save to localStorageService
    //     saveToLocalStorage($rootScope, localStorageService);
    //     $rootScope.onInit();
    //   } else {
    //     vm.stateCheckboxClick(state, false);
    //   }
    // }
    // var stateMap = "<img src='themes/addiction/images/" + state.image + ".png' style = 'width:100%;opacity:0.2'>";
    // vm.stateImage = state.image;
  ...
  vm.updateCategoriesModel = function (value, catgType) {
    ...
    if (catgType === 'demographic') {
      ...
      if (matched === 1) {
        ...
        $log.info('already there: ' + label + ' key: ' + key);
        $log.info(demographicModel);
  ...
  vm.highlightSelectedCategories = function () {
    ...
    $log.info(vm.categoriesActiveIds);
  };
  ...
  vm.updateStateSelect = function (state, stateCheck) {
    ...
    $log.info($rootScope.checkedStateDetail);
  };
  ...
  vm.deleteFromCart = function (item) {
    ...
    signupService.getCartDetails(token).then(function (result) {
      ...
      if (actualDelItemInfo !== '') {
        ...
        signupService.deleteSponsorAds(actualDelItemInfo.id, $rootScope.activeCenter, token).then(function (result) {
  ...
  vm.stateCheckboxClick = function (state, selectOnly, mapClicked) {
    // $log.info(vm.stateSelectCheck);
    // $log.info(state);
    ...
    if (angular.isUndefined(vm.stateSelectCheck)) {
      vm.stateSelectCheck = {};
      vm.stateSelectCheck[state.name] = true;
      $log.info('initialized stateSelectCheck');
    } else if (vm.stateSelectCheck[state.name] === true) {
      //  vm.stateSelectCheck[state.name] = false;
      vm.prevSelStyle[shortname] = 'color:#8f8686';
      vm.curSelStyle[shortname] = 'color:#015b8e;text-decoration:underline;';
    } else if (vm.stateSelectCheck[state.name] === false) {
      //  vm.stateSelectCheck[state.name] = true;
      vm.prevSelStyle[shortname] = '';
    ...
    //  $log.info('stateselectcheck: ' + vm.stateSelectCheck[state.name]);
    vm.updateStateSelect(state, vm.stateSelectCheck[state.name]);
    // save to localStorageService
    saveToLocalStorage($rootScope, localStorageService);
    $rootScope.onInit();
  };
  ...
  vm.loadCityCounty = function (state, mapOnly) {
    ...
    if (mapOnly !== 1) {
      getCountyCity(vm, state, '', token, service, $injector, $rootScope, localStorageService, Status);
    }
  };
  ...
  vm.selectAllState = function (selectall, key) {
    $log.info('key: ' + key);
    $log.info($rootScope.stateIds.length);
    ...
    if (selectall) {
      ...
      $log.info(shortname);
      $log.info(vm.stateSelectCheck[shortname]);
      if (angular.isDefined(vm.stateSelectCheck[shortname]) && vm.stateSelectCheck[shortname] === false) {
...
function getCountyCity(vm, state, stateMap, token, service, $injector, $rootScope, localStorageService, Status) {
  ...
  service.getCityCountyByState(token, state.shortname).then(function (response) {
    ...
    if (angular.isDefined(cartMode) && cartMode !== null) {
      if (cartMode.mode === 'edit' && cartMode.item === 'sponsored_layouts') {
        ...
        // if (angular.isUndefined($rootScope.cityModel[actCen])) {
        //   $rootScope.cityModel = {};
        //   $rootScope.cityModel[actCen] = [];
        // }
        // if (angular.isUndefined($rootScope.countyModel[actCen])) {
        //   $rootScope.countyModel = {};
        //   $rootScope.countyModel[actCen] = [];
        // }
        $rootScope.cityModel = {};
