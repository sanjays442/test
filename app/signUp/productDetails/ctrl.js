...
function ctrl($injector, $timeout, $scope, $log, Status, $rootScope, $state, UIState, service, localStorageService) {
  ...
  vm.testCenter = function () {
    ...
    localStorageService.remove('cartMode', 'cartReached', 'lastAddedCenter', 'addCenterProgress', 'membershipType', 'signupSponsoredPage', 'edit_bannerads', 'sessionStorage');
  ...
  function calculateSubtotals() {
    ...
    for (var item in vm.cartDetails.items) {
      ...
      vm.itemsIds[centerId] = [];
      vm.centerSubTotals[centerId] = vm.cartDetails.items[item].subtotal;
      // sponsored layouts
      for (var sl in vm.cartDetails.items[item].sponsored_layouts) {
        for (var itm in vm.cartDetails.items[item].sponsored_layouts[sl]) {
          ...
          vm.itemsIds[centerId].push(vm.cartDetails.items[item].sponsored_layouts[sl][itm].id);
        }
      }
      // banner ads
      for (var ads in vm.cartDetails.items[item].banner_ads) {
        ...
        vm.itemsIds[centerId].push(vm.cartDetails.items[item].banner_ads[ads].id);
      }
    }
    if (angular.isUndefined($rootScope.cameFromEdit) || $rootScope.cameFromEdit === 0) {
      $rootScope.beforeEditItemIds = null;
      vm.beforeEditItemIds = null;
    } else {
      delete $rootScope.cameFromEdit;
      vm.beforeEditItemIds = $rootScope.beforeEditItemIds;
      $rootScope.beforeEditItemIds = null;
    }
    localStorageService.remove('beforeEditItems');
  }
  ...
  vm.editCenter = function (cenId, cenName, item, index) {
    ...
    $rootScope.beforeEditItemIds = vm.itemsIds;
    delete $rootScope.cameFromEdit;
    $state.go(UIState.SIGN_UP.SPONSORED_PAGE);
    //  $state.go(UIState.SIGN_UP.SPONSORED_PAGE,{'centerId':cenId});
  };
  // for delete icon
  vm.deleteLoader = {};
  vm.deletedRow = {};
  vm.deleteSponsorAds = function (itemId, cenId) {
    ...
    service.deleteSponsorAds(itemId, cenId, token).then(function (result) {
  ...
  vm.deleteBannerAds = function (itemId, cenId) {
    ...
    // delete sponsored ads using itemId
    service.deleteBannerAds(itemId, cenId, token).then(function (result) {
      // $rootScope.$emit(Status.SUCCEEDED, 'Item Removed');
      vm.deleteLoader[itemId] = 0;
      vm.deletedRow[itemId] = 1;
      vm.loadCart(cenId, false);
