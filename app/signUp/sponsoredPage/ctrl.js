function ctrl($injector, $log, $scope, $state, UIState, $stateParams, $rootScope, $window, $document, Status, SponsorService, signUpService, localStorageService, $timeout) {
  ...
  vm.submitComplete = function () {
    ...
    for (var cen in $rootScope.centerSelected) {
      ...
      // conditionchanged above else statement was not before
    }
    ...
    vm.submitSingle = function (ci) {
      ...
      $log.info('going to submit ids to api...');
      ...
      SponsorService.editSponsorSignup(formData, cenId, token).then(function (res) {
        ...
        if (ci > (centerIds.length - 1)) {
          ...
          if (angular.isDefined($rootScope.beforeEditItemIds)) {
            $rootScope.cameFromEdit = 1; // used in only cart page to check if came after editing
  ...
  vm.currentCenter = localStorageService.get('current_center'); // it comes from test center step
  ...
  vm.loadCart = function () {
    ...
    signUpService.getCartDetails(token).then(function (result) {
      ...
      //    $rootScope.activeCenterInfo = vm.currentCenter[0]; // selects current center
      $rootScope.calculateItems(vm.cartInfo.items);

      var beforeEditItems = localStorageService.get('beforeEditItems'); // if page refreshed then also maintain value from cart for edit and show new item as highlighted feature
      if (angular.isDefined(beforeEditItems) && beforeEditItems != null) {
        $rootScope.beforeEditItemIds = beforeEditItems.beforeEditItem;
      }
    }).catch(function (err) {
      $log.info(err);
    });
  ...
  $rootScope.changeActiveCenter = function () {
    ...
    if (angular.isDefined($rootScope.beforeEditItemIds)) {
      $rootScope.cameFromEdit = 1; // used in only cart page to check if came after editing
      var beforeEditItems = {
        'beforeEditItem': $rootScope.beforeEditItemIds
      }
      localStorageService.set('beforeEditItems', beforeEditItems);
    }
    $state.reload();
  };
