module.exports = ['$location', '$anchorScroll', '$timeout', '$injector', '$document', '$scope', '$log', '$rootScope', '$state', 'UIState', 'SignUpService', 'localStorageService', 'Status', ctrl];
...
function ctrl($location, $anchorScroll, $timeout, $injector, $document, $scope, $log, $rootScope, $state, UIState, service, localStorageService, Status) {
  ...
  vm.submitAds = function (content, position, weblink, id) {
    ...
    if (angular.isDefined(vm.editCenId) && angular.isDefined(id)) {
      ...
      vm.bannerAdsEdit(formData, id);
      return;
    }
    $log.info('normal mode..');
    service.publishAds(formData, token, centerId).then(function (result) {
  ...
  vm.bannerAdsEdit = function (formData, adId) {
    service.publishAdsEdit(formData, token, adId, vm.editCenId).then(function (result) {
      $log.info(result);
      //  lm.$emit(Status.SUCCEEDED, 'Ads submitted');
      if (vm.count >= vm.validAds.length) {
        lm.$emit(Status.HIDE_PROCESSING, '');
